...
## as indicated by their Bonferroni corrected p-Value
################################
...
## (1) Sample1
## (2) Sample2
## (3) Number of assays with matching Genotype
## (4) Total number of assays
## (5) p-Value
## (6) Bonferroni Corrected p-Value
##
################################

def create_clusters(infile,max_p):
    ...
    with open(infile,'rb') as f:
        for line in f:
            if 'Sample' not in line:
                s1,s2,matches,total,pval,bon_pval = line.strip().split("\t")
                if float(bon_pval) <= max_p:
...
def cluster_samples(args):
    """
    Cluster samples that meet the max p-value threshold set by user. 
    ...
    if not len(args) == 3:
        print >> sys.stderr, "Usage: %prog infile outfile max_p"
        sys.exit(-1)

    infile,outfile,max_p = args
    max_p = float(max_p)
    print max_p
    clusters = create_clusters(infile,max_p)
