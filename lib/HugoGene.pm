...
# Used to hold data from the Hugo web site
sub new {
    my $proto = shift;
...
## Methods to see if this Hugo symbol refers to the same gene as identified
...
sub sameMajorChrBand {
    ...
    if ( $chr =~ /ter/i || $inputChr =~ /ter/i || $chr =~ /cen/i || $inputChr =~ /cen/i ) {
        if ( $chr =~ /(\w+)(p|q|cen)/i ) { $chr = $1; }
        if ( $inputChr =~ /(\w+)(p|q|cen)/i ) { $inputChr = $1; }
        return ( $inputChr eq $chr );
    }

    # Chromosomes can be in format '$chr[pq]\d+.\d+' or can be given as a range
    if ( $chr =~ /(\w+)([pq])(\d*)?/i ) {
        $chr = $1; $arm = $2; $band = $3;
    }
    ...
    if ( !defined $arm || !defined $inputArm || $arm eq "" || $inputArm eq "" ) {
        return 1;
    }
    ...
    # The chromosomes and arms are the same, allow the bands to differ by 1
    if ( defined $band && defined $inputBand && $band ne "" && $inputBand ne "") {
        return ( abs($band - $inputBand) <= 1 );
    }

    # The chromosomes are the same, one or more bands are not defined
    return 1;
}
...
sub sameChr {
    ...
    # This should not be called if $chr is not defined
    (defined $chr) or carp $self->symbol(), " does not have a defined chromosome";

    # Remove common junk
    $chr =~ s/chr//i; $inputChr =~ s/chr//i;
    $chr =~ s/\s+//g; $inputChr =~ s/\s+//g;
    # Change to uppercase in case chromosomes are X or Y
    $chr = uc $chr; $inputChr = uc $inputChr;
