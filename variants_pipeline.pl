...
my $targets_ilist = "$ASSAY_DEF/$targets/$targets\_targets.ilist";
my $baits_ilist = "$ASSAY_DEF/$targets/$targets\_baits.ilist";
my $targets_bed_padded = "$ASSAY_DEF/$targets/$targets\_targets_plus5bp.bed";
my $targets_facet = "$ASSAY_DEF/$targets/$targets\_targets_FACETS.ilist";
my $assay = $targets;
if(-d $targets){
		...
		if($noMD){
            ...
            if($rna)
            {
                if(!-e "$output/progress/$pre\_$uID\_SNCR_$samp\_$lib\.done" || $ran_md){
                    my %stdParams = (scheduler => "$scheduler", job_name => "$pre\_$uID\_SNCR_$samp\_$lib", job_hold => "$pre\_$uID\_$samp\_$lib\_MARKDUPS", cpu => "10", mem => "30", cluster_out => "$output/progress/$pre\_$uID\_SNCR_$samp\_$lib\.log");
                    my $standardParams = Schedule::queuing(%stdParams);
