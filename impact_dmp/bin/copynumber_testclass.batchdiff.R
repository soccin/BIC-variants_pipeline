...
d.stdn <- d.stdn[,which(grepl('STDN_R',colnames(d.stdn)))];
#if(prefix == 'IMPACTv3-CLIN-20140002'){
#	d.stdn <- d.stdn[,-grep('R2',colnames(d.stdn))];
#}else{
#	if(prefix == 'IMPACTv3-CLIN-20140005'){
#		d.stdn <- d.stdn[,-grep('R5',colnames(d.stdn))];
#	}
#}
d.stdn.batch <- unlist(strsplit(colnames(d.stdn),'\\.'));
d.stdn.batch <- d.stdn.batch[grep('STDN',d.stdn.batch)];
d.stdn.median <- do.call('rbind',list(apply(d.stdn,1,function(row.i){
	return(tapply(row.i,d.stdn.batch,median,na.rm=T));
})));
d.stdn.median <- t(d.stdn.median);
rownames(d.stdn.median) <- genelevel[,'Target'];
colnames(d.stdn.median) <- unique(d.stdn.batch);

# 2A - correct autosomal values if median is taken over 5 normals in batch
if(ncol(test) >= 5){
	test.median <- 	apply(test,1,median);
	autosome.mask <- rep(1,nrow(d.stdn.median));
	autosome.mask[which(grepl('^X\\:',rownames(d.stdn.median)) | grepl('^Y\\:',rownames(d.stdn.median)))] <- 0;
	batch.diff <- do.call('cbind',list(apply(d.stdn.median,2,function(col.i){
		ret <- test.median - col.i;
	...
	# DEBUG:
	#par(mfrow=c(1,2));
	#hist(batch.diff[,1],breaks=seq(-2,2,0.05),border='red',xlab='',ylab='',ylim=c(0,1500),main=paste('Batch 2 adjustment range',paste(round(quantile(batch.diff[,1],c(0.05,0.95),na.rm=T),3),collapse=' ~ '),sep='\n'));
	#hist(batch.diff[,2],breaks=seq(-2,2,0.05),border='grey20',xlab='',ylab='',ylim=c(0,1500),main=paste('Batch 5 adjustment range',paste(round(quantile(batch.diff[,2],c(0.05,0.95),na.rm=T),3),collapse=' ~ '),sep='\n'));

	d.stdn.adj <- do.call('cbind',lapply(colnames(d.stdn),function(colname.i){
		batch <- unlist(strsplit(colname.i,'\\.'))[1];
		return(as.numeric(as.character(d.stdn[,colname.i]))+batch.diff[,batch]);
	}));
	colnames(d.stdn.adj) <- colnames(d.stdn);
	d.stdn <- d.stdn.adj;
}

#2B - 
# Get gender info for each std.normal
stdn.gender <- stdn_cov.title[match(colnames(d.stdn),paste('STDN',stdn_cov.title[,'Barcode'],sep='_')),'Sample_type'];
names(stdn.gender) <- colnames(d.stdn);
Xsome.mask <- rep(0,nrow(d.stdn.median));
Xsome.mask[which(grepl('^X\\:',rownames(d.stdn.median)))] <- 1;
if(sum(gender == 'M') >= 5){
	d.stdn.M.median <- do.call('rbind',list(apply(d.stdn[,which(stdn.gender == 'M')],1,function(row.i){
		return(tapply(row.i,d.stdn.batch[which(stdn.gender == 'M')],median,na.rm=T));
	})));
	d.stdn.M.median <- t(d.stdn.M.median);
	rownames(d.stdn.M.median) <- genelevel[,'Target'];
	colnames(d.stdn.M.median) <- unique(d.stdn.batch[which(stdn.gender == 'M')]);
	
	test.median.M <- apply(test[,which(gender == 'M')],1,median);
	batch.diff.M <- do.call('cbind',list(apply(d.stdn.M.median,2,function(col.i){
		ret <- test.median.M - col.i;
		ret <- ret * Xsome.mask;
		return(ret);
	})));
	...
	d.stdn.adj <- do.call('cbind',lapply(colnames(d.stdn),function(colname.i){
		if(stdn.gender[colname.i] != 'M'){
			return(as.numeric(as.character(d.stdn[,colname.i])));
		}else{
			batch <- unlist(strsplit(colname.i,'\\.'))[1];
			return(as.numeric(as.character(d.stdn[,colname.i]))+batch.diff.M[,batch]);
		}
	}));
	colnames(d.stdn.adj) <- colnames(d.stdn);
	d.stdn <- d.stdn.adj;
...
if(sum(gender == 'F') >= 5){
	d.stdn.F.median <- do.call('rbind',list(apply(d.stdn[,which(stdn.gender == 'F')],1,function(row.i){
		return(tapply(row.i,d.stdn.batch[which(stdn.gender == 'F')],median,na.rm=T));
	})));
	d.stdn.F.median <- t(d.stdn.F.median);
	rownames(d.stdn.F.median) <- genelevel[,'Target'];
	colnames(d.stdn.F.median) <- unique(d.stdn.batch[which(stdn.gender == 'F')]);
	
	test.median.F <- apply(test[,which(gender == 'F')],1,median);
	batch.diff.F <- do.call('cbind',list(apply(d.stdn.F.median,2,function(col.i){
		ret <- test.median.F - col.i;
		ret <- ret * Xsome.mask;
		return(ret);
	})));
	...
	d.stdn.adj <- do.call('cbind',lapply(colnames(d.stdn),function(colname.i){
		if(stdn.gender[colname.i] != 'F'){
			...
			batch <- unlist(strsplit(colname.i,'\\.'))[1];
			return(as.numeric(as.character(d.stdn[,colname.i]))+batch.diff.F[,batch]);
		}
	}));
	colnames(d.stdn.adj) <- colnames(d.stdn);
	d.stdn <- d.stdn.adj;
