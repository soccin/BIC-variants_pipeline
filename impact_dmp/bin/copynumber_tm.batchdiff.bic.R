...
y.cov <- do.call('c',list(apply(normal[which(genelevel[,'Target'] %in% y.probes),colnames(d.stdn)],2,function(col.i){
...
tumor <- do.call('cbind',list(apply(genelevel[,title[index,'Sample_ID'],drop=F],2,function(x){
      ret <- as.numeric(x);
...
patient.TN <- do.call('rbind',lapply(patients,function(pt.i){
	...
	if(any(grepl('Gender',colnames(title)))){
		gender <- as.character(title[which(title[,'Patient_ID'] == pt.i & title[,'Class'] %in% c('Tumor')),'Gender'])[1];
		if(gender %in% c('Male','M','male')){
			gender <- 'M';			
		}else if(gender %in% c('Female','F','female')){
			gender <- 'F';
		}	
	}
...
analysis.out <- do.call('rbind',lapply(seq(1,nrow(patient.TN),1),function(i){
	...
	cat(pt,"\t",tm,"\n");
	tm.probes <- tumor[,tm];
	names(tm.probes) <- genelevel[,'Target'];
	...
	if(is.na(nm)){
		...
		nm.probes <- normal[,nm];
		names(nm.probes) <- genelevel[,'Target'];
	}
	...
	y.cov.pt <- median(as.numeric(tm.probes[grep('^Y\\:',genelevel[,'Target'])]),na.rm=T);
	if(gender != '-'){
		pt.gender <- gender;
	}else{
	...
	#	if(pt.gender %in% c('M', 'F')){ #if sample gender is known 
	...
	#}
	#else{                           #if sample gender is inferred
	cn.probes.X <- cn.probes.X[,which(colnames(cn.probes.X) %in% c(nm,paste('STDN_',stdn_cov.title[which(stdn_cov.title[,'Sample_type'] == toupper(pt.gender)),'Barcode'],sep=''))),drop=F];
	#}
	...
	# Best-normal check			
	ratiolabel <- paste(tm," vs. ",best.norm.auto,"\nSex=",pt.gender,", Norm for X=",best.norm.X,"\nIf 'Sex' above is in lower case 'm' or 'f', then it's inferred from sequencing data, you should NOT trust result on chrX", sep="");
	if(doFull == 'FULL'){
	    sig.list <- cn.analysis(tm.probes, best.nm.probes.auto, best.nm.probes.X, tm, bc, genelabel,make.plot=T);
	}else{
        sig.list <- cn.analysis(tm.probes, best.nm.probes.auto, best.nm.probes.X, tm, bc, genelabel,make.plot=F);
    }
	...
	idx <- which(!grepl('Tiling',sig.gene.gains[,'genes']) & as.numeric(sig.gene.gains[,'fc']) >= 2.0 & as.numeric(sig.gene.gains[,'p.adj']) < 0.05);	   
	if(length(idx)>0){
	...
	idx <- which(!grepl('Tiling',sig.gene.losses[,'genes']) & as.numeric(sig.gene.losses[,'fc']) <= -2.0 & as.numeric(sig.gene.losses[,'p.adj']) < 0.05);	   
	if(length(idx)>0){
