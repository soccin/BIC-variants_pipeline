...
#prefix <- 'IMPACT-VAL-Run12Acv3HiSeq02';
#outdir <- '/Volumes/phoenix-home/chengd1/LabPipeline/CopyNumberWork';
#gcfile <- 'all_GC150bp.txt';
...
exon_file <- file.path(outdir,paste(prefix,"_ALL_exoncoverage.txt",sep=""));
tiling_file <- file.path(outdir,paste(prefix,"_ALL_tilingcoverage.txt",sep=""));
interval_file <- file.path(outdir,paste(prefix,"_ALL_intervalcoverage.txt",sep=""));
...
outtablename <- file.path(outdir,paste(prefix,"_ALL_intervalcoverage_loess.txt",sep=""));
...
norm_rt <- do.call('cbind',lapply(seq(1,ncol(gc2),1),function(i){
	...
	normalized.filt<-(column_sqrt-temp2+median(column_sqrt))/(median(column_sqrt));
	...
	normalized.tiling<-(column_sqrt.tiling.inclY-temp2.tiling+median(column_sqrt))/(median(column_sqrt));
