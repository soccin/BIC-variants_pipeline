...
#source('textplot.R');
#prefix <- 'IMPACTv3-CLIN-20140007-8';
#stdnormal_loess <- 'BloodStdNormals';
...
index <- which(title$Class=="Normal"); #selects the normal barcodes 
if(length(index)==0){
	res0 <- c('No normals in pool');
	write.table(res0, file=paste(prefix,"_normvsnorm_results.txt", sep=""),sep="\t",row.names=F,col.names=T,quote=F); 
	stop('No normals in pool\n');
}
normal <- do.call('cbind',list(apply(genelevel[,title[index,'Sample_ID'],drop=F],2,as.numeric)));  
gender <- rep('-',ncol(normal));
...
doMedianOnCols <- function(dat){
	normal.median <- apply(dat,2,median);
...
y.cov <- do.call('c',list(apply(d.stdn[grep('^Y\\:',genelevel[,'Target']),],2,function(col.i){
	return(median(as.numeric(col.i),na.rm=T));
})));
y.cov_grp <- abs(tapply(y.cov,stdn_cov.title[match(gsub('STDN_','',names(y.cov)),stdn_cov.title[,'Barcode']),'Sample_type'],mean,na.rm=T));	
   
sum.na <- apply(normal,2,function(x){return(sum(is.na(x)));});
index <- which(sum.na > 100);
if(length(index)>1){
    normal <- normal[,-index,drop=F];
}
...
seg.clus.p <- function(logratio, genelabel, make.plots=F, sample.id){
	...
	segment.smoothed.cna.obj.srt <- do.call('rbind',lapply(c(1:22,"X"),function(chr){
		return(segment.smoothed.cna.obj$output[which(segment.smoothed.cna.obj$output[,'chrom'] == chr),,drop=F]);
	}));
	...
	if(make.plots){
		par(mfrow=c(2,2));
		plot(seg.probes[order(seg.probes,decreasing=F)],main='Unclustered seg values',
	    	 xlim=c(1,length(seg.probes)),xlab="",
		     ylim=c(min(seg.probes),max(seg.probes)),ylab="");
	...
	ret.intragenic <- do.call('rbind',lapply(all.genes,function(gene.i){
		...
		fc.x <- fc[names(genelabel)[which(genelabel == gene.i)]];
		p.adj.x <- p.adj[names(genelabel)[which(genelabel == gene.i)]];
		...
		if(length(lr.x) >= 3 & length(unique(lr.x)) > 2 & sum(is.na(lr.x))==0 & length(table(sig.x)) > 1){
			...
			if(ratio >= 4){		
				...
				if((k2$size[clus.del] == 1 & clus.diff >= 1.0) |
				   (k2$size[clus.del] > 1 & clus.diff >= 0.5 & pct.stretch > 0.5)){	
	...
	if(is.null(ret.intragenic)){			   
		return(rbind(ret.probes,ret.genes));
	}else{
		return(rbind(ret.probes,ret.genes,ret.intragenic));
	}
...
cn.analysis <- function(tm.vec, auto_nm.vec, x_nm.vec, tm.label, genelabel,make.plot=F){
	...
	if(is.na(x_nm.vec)){
		lr[x.idx] <- NA; # replace with unmatched normal of same sex
	}else{
...
out_file= paste(prefix,"_normvsnorm_segclusp.pdf", sep="");
...
text(1, 8, "Copy number segmentation analysis: Normal vs Normal QC",adj=0,cex=1.5,font=2)
...
analysis <- do.call('rbind',lapply(all.normals,function(nm){
	nm.probes <- normal[,nm];
	...
	cn.probes <- do.call('cbind',lapply(colnames(d.stdn),function(snm){		
		...
		ret.probes <- calc.fc(nm.probes,nm.probes.i);
	...
	y.cov.pt <- median(as.numeric(nm.probes[grep('^Y\\:',genelevel[,'Target'])]),na.rm=T);
	if(gender[nm] != '-'){
		pt.gender <- gender[nm];
	}else{
	...
	# Best-normal check			
	ratiolabel <- paste(nm," vs. ",best.norm.auto,"\nSex=",pt.gender,", Norm for X=",best.norm.X,sep="");

    sig.list <- cn.analysis(nm.probes, best.nm.probes.auto, best.nm.probes.X, nm, genelabel,make.plot=F);
	...
	sig.genes[,'fc'] <- as.numeric(as.character(sig.genes[,'fc']));
	sig.genes[,'p.adj'] <- as.numeric(as.character(sig.genes[,'p.adj']));
	...
	lr.plot_tiling <- lr;
	lr.plot_exon <- lr;
	lr.plot_tiling[-which(rownames(sig.probes) %in% tiling_probe_ids)] <- NA;
	lr.plot_exon[which(rownames(sig.probes) %in% tiling_probe_ids)] <- NA;
	...
	ret <- cbind('sample'=rep(nm,nrow(sig.list)),
...
#write output of analysis
write.table(analysis, file=paste(prefix,"_normvsnorm_results.txt", sep=""),sep="\t",row.names=F,col.names=T,quote=F);
