...
#setwd('/Volumes/phoenix-home/chengd1/LabPipeline/CopyNumberWork/nomapq');
#source('/Volumes/phoenix-home/chengd1/LabPipeline/CopyNumberWork/textplot.R');
#prefix <- 'IMPACTv3-CLIN-20140002';
...
covfile_tiling <-"_ALL_tilingnomapqcoverage.txt";
covfile_loess <-"_ALL_intervalnomapqcoverage_loess.txt";
...
chr <- do.call('c',lapply(stdn_cov[,'Target'],function(x){
	f <- unlist(strsplit(x,'\\:'));
	return(f[1]);
}));
...
if(length(index)==0){
	...
	print('No tested classes in pool\n');
	quit(status = 0);
}
...
if(any(grepl('Gender',colnames(title)))){
	...
	idx2 <- which(gender %in% c('Female','F','female'));
	if(length(idx)!=0){
		gender[idx] <- 'M';
	}
	if(length(idx2)!=0){
...
# 1. Remove exons with systematically low coverage
...
# 2. Correct samples for batch effects 
# - correct X chromosome probes in gender specific manner. Requires 5 gender-specific normals in each batch
d.stdn.bld  <- d.stdn[,which(grepl('STDN_R',colnames(d.stdn)))];
d.stdn.ffpe <- d.stdn[,which(grepl('STDN_FFPE',colnames(d.stdn)))];
d.stdn.other <- d.stdn[,which(!grepl('STDN_FFPE',colnames(d.stdn)) & !grepl('STDN_R',colnames(d.stdn)))];

# if exists >= 5 blood normals in pool, batch correct std blood normals to match current pool.

idx <- which(title[,'Class'] == 'Normal' & title[,'Sample_type'] %in% c('Blood','Frozen','Fresh','OCT'));
if(length(idx) >= 3 & ncol(d.stdn.bld) >=3){
	pt.nm <- genelevel[,title[idx,'Sample_ID']];
	pt.nm.median <- apply(pt.nm,1,median);
	autosome.mask <- rep(1,length(pt.nm.median));
	autosome.mask[which(grepl('^X\\:',genelevel[,'Target']) | grepl('^Y\\:',genelevel[,'Target']))] <- 0;
	
	d.stdn.bld.batch <- unlist(strsplit(colnames(d.stdn.bld),'\\.'));
	d.stdn.bld.batch <- d.stdn.bld.batch[grep('STDN',d.stdn.bld.batch)];
	d.stdn.bld.batch <- factor(d.stdn.bld.batch, levels = unique(d.stdn.bld.batch));
	d.stdn.bld.median <- do.call('rbind',list(apply(d.stdn.bld,1,function(row.i){
		return(tapply(row.i,d.stdn.bld.batch,median,na.rm=T));
	})));
	d.stdn.bld.median <- t(d.stdn.bld.median);
	rownames(d.stdn.bld.median) <- genelevel[,'Target'];
	colnames(d.stdn.bld.median) <- levels(d.stdn.bld.batch);
	...
	d.stdn.bld.adj <- do.call('cbind',lapply(colnames(d.stdn.bld),function(colname.i){
		...
		return(as.numeric(as.character(d.stdn.bld[,colname.i]))+batch.diff[,batch]);
	}));
	...
	bld.gender <- do.call('c',lapply(title[match(colnames(pt.nm),title[,'Sample_ID']),'Gender'],function(x){
		if(x %in% c('Male','M','male')){
			return('M');
		}else if(x %in% c('Female','F','female')){
			return('F');
		}else{
			return('-');
		}
	...
	Xsome.mask[which(grepl('^X\\:',genelevel[,'Target']))] <- 1;
	if(sum(bld.gender == 'M') >= 3){
		d.stdn.bldM.median <- do.call('rbind',list(apply(d.stdn.bld[,which(stdn.gender == 'M'),drop=F],1,function(row.i){
			return(tapply(row.i,d.stdn.bld.batch[which(stdn.gender == 'M')],median,na.rm=T));
		})));
		d.stdn.bldM.median <- t(d.stdn.bldM.median);
		rownames(d.stdn.bldM.median) <- genelevel[,'Target'];
		colnames(d.stdn.bldM.median) <- levels(d.stdn.bld.batch);	
	
		pt.nm.median.M <- apply(pt.nm[,which(bld.gender == 'M')],1,median);
	...
	if(sum(bld.gender == 'F') >= 3){
		d.stdn.bldF.median <- do.call('rbind',list(apply(d.stdn.bld[,which(stdn.gender == 'F'),drop=F],1,function(row.i){
			return(tapply(row.i,d.stdn.bld.batch[which(stdn.gender == 'F')],median,na.rm=T));
		})));
		d.stdn.bldF.median <- t(d.stdn.bldF.median);
		rownames(d.stdn.bldF.median) <- genelevel[,'Target'];
		colnames(d.stdn.bldF.median) <- levels(d.stdn.bld.batch);	
	
		pt.nm.median.F <- apply(pt.nm[,which(bld.gender == 'F')],1,median);
...
idx <- which(title[,'Class'] == 'Normal' & title[,'Sample_type'] %in% c('FFPE','Cytology'));
if(length(idx) >= 3 & ncol(d.stdn.ffpe) >=3){
        pt.nm <- genelevel[,title[idx,'Sample_ID']];
        pt.nm.median <- apply(pt.nm,1,median);
        autosome.mask <- rep(1,length(pt.nm.median));
        autosome.mask[which(grepl('^X\\:',genelevel[,'Target']) | grepl('^Y\\:',genelevel[,'Target']))] <- 0;

        d.stdn.ffpe.batch <- unlist(strsplit(colnames(d.stdn.ffpe),'\\.'));
        d.stdn.ffpe.batch <- d.stdn.ffpe.batch[grep('STDN',d.stdn.ffpe.batch)];
        d.stdn.ffpe.batch <- factor(d.stdn.ffpe.batch, levels = unique(d.stdn.ffpe.batch));
        d.stdn.ffpe.median <- do.call('rbind',list(apply(d.stdn.ffpe,1,function(row.i){
                return(tapply(row.i,d.stdn.ffpe.batch,median,na.rm=T));
        })));
        d.stdn.ffpe.median <- t(d.stdn.ffpe.median);
        rownames(d.stdn.ffpe.median) <- genelevel[,'Target'];
        colnames(d.stdn.ffpe.median) <- levels(d.stdn.ffpe.batch);

        batch.diff <- do.call('cbind',list(apply(d.stdn.ffpe.median,2,function(col.i){
                ret <- pt.nm.median - col.i;
                ret <- ret * autosome.mask;
                return(ret);
        })));

        d.stdn.ffpe.adj <- do.call('cbind',lapply(colnames(d.stdn.ffpe),function(colname.i){
                batch <- unlist(strsplit(colname.i,'\\.'))[1];
                return(as.numeric(as.character(d.stdn.ffpe[,colname.i]))+batch.diff[,batch]);
        }));
        colnames(d.stdn.ffpe.adj) <- colnames(d.stdn.ffpe);
        d.stdn.ffpe <- d.stdn.ffpe.adj;

        stdn.gender <- stdn_cov.title[match(colnames(d.stdn.ffpe),paste('STDN',stdn_cov.title[,'Barcode'],sep='_')),'Sample_type'];
        names(stdn.gender) <- colnames(d.stdn.ffpe);
        ffpe.gender <- do.call('c',lapply(title[match(colnames(pt.nm),title[,'Sample_ID']),'Gender'],function(x){
                if(x %in% c('Male','M','male')){
                        return('M');
                }else if(x %in% c('Female','F','female')){
                        return('F');
        ...
        Xsome.mask <- rep(0,nrow(d.stdn.ffpe));
        Xsome.mask[which(grepl('^X\\:',genelevel[,'Target']))] <- 1;
        if(sum(ffpe.gender == 'M') >= 3){
                d.stdn.ffpeM.median <- do.call('rbind',list(apply(d.stdn.ffpe[,which(stdn.gender == 'M'),drop=F],1,function(row.i){
                        return(tapply(row.i,d.stdn.ffpe.batch[which(stdn.gender == 'M')],median,na.rm=T));
                })));
                d.stdn.ffpeM.median <- t(d.stdn.ffpeM.median);
                rownames(d.stdn.ffpeM.median) <- genelevel[,'Target'];
                colnames(d.stdn.ffpeM.median) <- levels(d.stdn.ffpe.batch);

                pt.nm.median.M <- apply(pt.nm[,which(ffpe.gender == 'M')],1,median);
                batch.diff.M <- do.call('cbind',list(apply(d.stdn.ffpeM.median,2,function(col.i){
                        ret <- pt.nm.median.M - col.i;
                        ret <- ret * Xsome.mask;
                        return(ret);
                })));
                ...
                d.stdn.ffpe.adj <- do.call('cbind',lapply(colnames(d.stdn.ffpe),function(colname.i){
                        if(stdn.gender[colname.i] != 'M'){
                                return(as.numeric(as.character(d.stdn.ffpe[,colname.i])));
                        }else{
                                batch <- unlist(strsplit(colname.i,'\\.'))[1];
                                return(as.numeric(as.character(d.stdn.ffpe[,colname.i]))+batch.diff.M[,batch]);
                        }
                }));
                colnames(d.stdn.ffpe.adj) <- colnames(d.stdn.ffpe);
                d.stdn.ffpe <- d.stdn.ffpe.adj;
        }
        if(sum(ffpe.gender == 'F') >= 3){
                d.stdn.ffpeF.median <- do.call('rbind',list(apply(d.stdn.ffpe[,which(stdn.gender == 'F'),drop=F],1,function(row.i){
                        return(tapply(row.i,d.stdn.ffpe.batch[which(stdn.gender == 'F')],median,na.rm=T));
                })));
                d.stdn.ffpeF.median <- t(d.stdn.ffpeF.median);
                rownames(d.stdn.ffpeF.median) <- genelevel[,'Target'];
                colnames(d.stdn.ffpeF.median) <- levels(d.stdn.ffpe.batch);

                pt.nm.median.F <- apply(pt.nm[,which(ffpe.gender == 'F')],1,median);
                batch.diff.F <- do.call('cbind',list(apply(d.stdn.ffpeF.median,2,function(col.i){
                        ret <- pt.nm.median.F - col.i;
                        ret <- ret * Xsome.mask;
                        return(ret);
                })));
                ...
                d.stdn.ffpe.adj <- do.call('cbind',lapply(colnames(d.stdn.ffpe),function(colname.i){
                        if(stdn.gender[colname.i] != 'F'){
                                ...
                                batch <- unlist(strsplit(colname.i,'\\.'))[1];
                                return(as.numeric(as.character(d.stdn.ffpe[,colname.i]))+batch.diff.F[,batch]);
                        }
                }));
                colnames(d.stdn.ffpe.adj) <- colnames(d.stdn.ffpe);
                d.stdn.ffpe <- d.stdn.ffpe.adj;
...
d.stdn <- cbind(d.stdn.bld,d.stdn.ffpe,d.stdn.other);
...
seg.clus.p <- function(logratio, genelabel, make.plots=F,sample.id,bc){
	...
	write.table(segment.smoothed.cna.obj.srt, 
			    paste(sample.id,"_",bc,"_",prefix,"_copynumber.seg",sep=""),
		...
		if(length(lr.x) >= 3 & length(unique(lr.x)) > 2 & length(table(sig.x)) > 1){ 
			...
			if(length(lr.x) >= 3 & length(unique(lr.x)) > 2 & length(table(sig.x)) > 1){ 
				...
				if(ratio >= 3.5){
					...
					if((k2$size[clus.gain] == 1 & clus.diff >= 0.25) |
				   	   (k2$size[clus.gain] > 1 & k2$size[clus.gain] < 6  & clus.diff >= 0.20 & pct.stretch > 0.5) |
				           (k2$size[clus.gain] >=6 & clus.diff >= 0.20 & pct.stretch >= 0.5)){								
...
analysis.out <- do.call('rbind',lapply(test.samples,function(tst){
	...
	d.stdn.selected <- d.stdn; 
	if(title[title[,'Sample_ID']==tst,'Sample_type'] %in% c('Blood','Frozen','Fresh','OCT') & ncol(d.stdn.bld) >= 3)
	{
		d.stdn.selected <- d.stdn.bld;
		print("blood standard normal will be used");
	}
	else if(title[title[,'Sample_ID']==tst,'Sample_type'] %in% c('FFPE','Cytology') & ncol(d.stdn.ffpe) >=3)
	{
		d.stdn.selected <- d.stdn.ffpe;
		print("ffpe standard normal will be used");
	}
	else
	{
		print("all standard normal will be used");
	}

	cn.probes <- do.call('cbind',lapply(colnames(d.stdn.selected),function(snm){		
	...
	cn.probes.X <- cn.probes.X[,which(colnames(cn.probes.X) %in% c(paste('STDN_',stdn_cov.title[which(stdn_cov.title[,'Sample_type'] == toupper(pt.gender)),'Barcode'],sep=''))),drop=F];				
	...
	if (a > 3){ a=signif(a, digits = 1)}else {a= 3}
	lr.plot_tiling <- lr;
	lr.plot_exon <- lr;
