...
#setwd('/Volumes/phoenix-home/chengd1/LabPipeline/CopyNumberWork/HemePact');
#source('/Volumes/phoenix-home/chengd1/LabPipeline/CopyNumberWork/textplot.R');

#source('/ifs/data/zeng/dmp/resources/dmp-cnv/bin/textplot.R');
source('textplot.R');
args <- commandArgs();
prefix <- args[5];
stdnormal_loess <- args[6];
...
#prefix <- 'Proj_HemePACT585_StdNormals';
#stdnormal_loess <- 'Proj_HemePACT585_StdNormals';
#gene_intervals.ann <- 'combined_gene_intervals.list.annotated';
#tiling_intervals.ann <- 'combined_tiling_interval.list.annotated';
#doFull <- 'MIN';
...
#if(any(grepl('STDN_bc10',colnames(d.stdn)))){
#	d.stdn <- d.stdn[,which(colnames(d.stdn) != "STDN_bc10")];
#}
...
normal.median <- apply(normal,2,median);
...
#Tumor
index <- which(title$Class %in% c("Tumor","NormalGL")); #selects the tumor barcodes 
...
analysis.out <- do.call('rbind',lapply(seq(1,length(all.normals),1),function(i){	
	...
	for(nm in all.normals){
		if(nm != tm){			
			cat(tm,"\t",nm,"\n");
			best.norm.auto <- nm;		
			best.nm.probes.auto <- normal[,nm];
			names(best.nm.probes.auto) <- genelevel[,'Target'];
	
			best.norm.X <- nm;
			best.nm.probes.X <- normal[,nm];
