...
`$JAVA/java -Xms256m -Xmx60g -XX:-UseGCOverheadLimit -Djava.io.tmpdir=/scratch/$uID -jar $ABRA/abra.jar --in $inBams --out $outBams --ref $refSeq --bwa-ref $bwaRef --targets $targets --working $working --threads 12 --mnf 5 --mbq 150 > $log 2>&1`;
